...
主界面集中显示设备连接、当前传输和接收缓存。主控端按已配置的屏幕位置排列设备，副机端显示已知设备及配置的主控地址；未知地址不会显示为实际连接地址。连接配置和日志可展开，窗口可自由缩放，小窗口保留滚动入口。设置分为“连接、文件传输、窗口、高级”四页。

0.3.1 压缩设备卡片和面板间距，合并底部状态控件；空闲时隐藏进度条，常规说明与缓存路径可通过悬停查看。异常详情仍直接显示。
...
当前 FileCopy 版本为 `0.3.1`，对应标签 `filecopy-v0.3.1`；下载以 Releases 实际附件为准。应用程序名称、底层协议和配置标识仍沿用 Deskflow；FileCopy 发行版本与“关于”界面的上游版本号分别记录。
...
.\deploy\windows\package-filecopy.ps1 `
  ...
  -OutputDirectory D:\Releases\deskflow-filecopy-0.3.1 `
  -Version 0.3.1 `
