...
主界面集中显示设备连接、当前传输和接收缓存。主控端按已配置的屏幕位置排列设备，副机端显示已知设备；设备卡片只显示当前连接实际使用的本机与对端 IP，未连接时隐藏 IP，不显示其他网卡或配置地址。连接配置和日志可展开，小屏保留滚动入口。设置分为“连接、文件传输、窗口、高级”四页。

0.3.2 将标题与连接状态合为一行，设备卡片进一步收紧，连接配置移到底部。主窗口默认宽度为 560，高度随内容调整：传输和展开日志时扩展，结束或收起后回收留白。空闲时隐藏进度条，常规说明与缓存路径可通过悬停查看，异常详情仍直接显示。

### 开始使用

1. 两端使用本项目的同一版本，先按 Deskflow 原有方式配置服务端、客户端和屏幕位置。
2. 两端启用 TLS 加密，并核对、信任对端指纹；服务端配置中启用剪贴板共享。
3. 两端打开“设置 → 文件传输”，勾选“启用文件复制”。此功能默认关闭。
...
当前 FileCopy 版本为 `0.3.2`，对应标签 `filecopy-v0.3.2`；下载以 Releases 实际附件为准。应用程序名称、底层协议和配置标识仍沿用 Deskflow；FileCopy 发行版本与“关于”界面的上游版本号分别记录。
...
接收缓存默认位于 `%LOCALAPPDATA%\Deskflow\file-transfer`，在通常的 Windows 用户配置中位于 **C 盘**。可通过主页面“管理缓存”进入“设置 → 文件传输 → 管理文件缓存”，选择其他本地磁盘上的专用目录，例如 `D:\DeskflowCache`，并设置容量、查看占用或清理缓存。保存设置后重新连接，新的接收任务才使用新位置和配额。安装程序的路径选择不会自动改变缓存位置。主页占用在后台统计，接收完成或关闭缓存设置后刷新。
...
0.3.0 已通过 Windows Release 构建和 **36 个 CTest 测试套件**，包含缓存保护、清理失败、进度估算、300 个小/空文件、长路径，以及新增界面的取消/中断、缓存异常、设置保存/取消和真实语言切换。完整运行中依赖特殊环境的磁盘不足、符号链接和映射网络盘用例会按环境跳过；对应环境补测及双机验收结果见各版本 Release 说明。

0.3.1 的界面收紧通过缓存、文件传输、设置对话框 3 个 GUI 回归套件，并检查了中英文、明暗主题及窄窗口的离线原生 Qt 渲染。
...
.\deploy\windows\package-filecopy.ps1 `
  ...
  -OutputDirectory D:\Releases\deskflow-filecopy-0.3.2 `
  -Version 0.3.2 `
