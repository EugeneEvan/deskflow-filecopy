...
反向操作相同。支持普通文件、多选、文件夹、空目录和中文文件名。传输完成前请等待，不要提前粘贴。主窗口状态栏显示进度并提供取消按钮；校验完成后才会发布接收文件到剪贴板。
...
首个 FileCopy 发行版本为 `0.1.0`，对应标签 `filecopy-v0.1.0`。应用程序名称、底层协议和配置标识仍沿用 Deskflow；FileCopy 发行版本与“关于”界面的上游版本号分别记录。

安装程序使用独立的 Deskflow FileCopy 产品标识，可选择安装路径；请使用独立目录，避免覆盖原版 Deskflow。默认不自动启动程序、不设置开机启动。当前安装程序未签名，Windows 可能提示“未知发布者”，请核对下载来源及该版本的校验值。
...
| 缓存配额 | 20 GiB，达到配额时拒绝新的接收任务 |
| 同名选择 | 同一批次中同名的顶层文件或目录会被拒绝，不自动合并或重命名 |
| 同时复制 | 两端同时发起的冲突任务会取消，请重新复制 |
| 不支持 | 跨机剪切、拖放、断点续传、符号链接/目录联接等重解析点、跨平台文件互传、多个客户端之间经服务端中转 |

接收缓存位于 `%LOCALAPPDATA%\Deskflow\file-transfer`，在通常的 Windows 用户配置中位于 **C 盘**。把应用或开发工具安装到其他盘不会改变这个缓存位置。

成功接收的批次**不会自动清理**，以免破坏尚未完成的粘贴。完成复制、确认无需再次粘贴且没有正在进行的传输后，可手动清理该缓存目录；清理后不可继续粘贴其旧剪贴板引用。当前尚未提供缓存位置设置和自动过期清理。

不同接收批次的缓存相互隔离。最终目标文件夹已有同名文件时，由 Windows 粘贴界面决定替换、跳过或重命名。

## 验证范围

已有完整 Windows 构建与 **31 个 CTest 测试套件通过**的记录。其中符号链接拒绝用例因测试账户无创建符号链接权限而跳过 1 项，不能视为该边界已经实测通过。
...
.\deploy\windows\package-filecopy.ps1 `
  ...
  -OutputDirectory D:\Releases\deskflow-filecopy-0.1.0 `
  -Version 0.1.0 `
